...
For the **BACnet B-SA (Smart Actuator) C++ example** -
...
| **Product Name** | CAS BACnet Stack Example - B-SA |
| **Product Model Number** | CAS BACnet Stack Example - B-SA |
| **Application Software Version** | 1.0.0 |
| **Firmware Revision** | 1.0.0 |
| **BACnet Protocol Version** | 1 |
| **BACnet Protocol Revision** | 24 |

**Product Description:** a minimal BACnet/IP actuating device built on the CAS
BACnet Stack. It presents three read-only sensor objects (Analog Input, Binary
Input, Multi-State Input) and three commandable output objects (Analog Output,
Binary Output, Multi-State Output), answers ReadProperty for every required
property of every object, accepts WriteProperty to the outputs' `Present_Value`,
and is discoverable by Who-Is / I-Am and Who-Has / I-Have. It is a tutorial for
implementers of the B-SA profile.

## 2. BACnet standardized device profile (Annex L)

**B-SA - BACnet Smart Actuator.**

This device claims exactly one profile. Because the B-SA requirements are a
superset of B-GENERAL's, a conformant B-SA device also satisfies **B-GENERAL**
...
scheduling (SCHED-*), trending (T-*), or DM-DCC-B (DeviceCommunicationControl).
...
Any other confirmed service is rejected. That rejection is part of the profile
...
No object is dynamically creatable or deletable. `Present_Value` is writable on
the three output objects (through their `Priority_Array`); no other property of
any object is writable.

| Object type | Instance | Object_Name | Optional properties supported |
|---|:---:|---|---|
| Device | 389002 | Rainbow | Description |
