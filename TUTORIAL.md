# Tutorial - extending and reviewing the B-SA example
...
> answering. This example never has a case where it knows a read is wrong, so
> every `Get*` callback declines with `errorCode` left alone throughout — the
> table above is still how the fall-through behaves.
>
...
(see the long comment above that loop in `main.cpp` — for Analog/Binary/Multi-State
Output those three calls are no-ops that document the contract, but the same
pattern becomes load-bearing on an optionally-commandable type like Analog Value).
Then read back every required property and diff against the existing output,
exactly as for the input recipe above — the same silent-default trap applies.
...
from the `Priority_Array` slots your `GetPropertyBool`/typed getters return (see
the `Commandable` struct in `main.cpp`). Add a new output instance to the
`outputs[]` table in `main` and to `GetCommandable()`, or it will not be
commandable.
...
For a commandable output such as Analog Output 1, the same picture but for
`Present_Value`, `Priority_Array` and `Current_Command_Priority`: all three are
**stack**, computed from the `Priority_Array` slots your `GetPropertyBool` /
typed getters report through `ReadPrioritySlot()`. Your code owns the storage
(the `Commandable` struct); the stack owns the resolution logic (highest
non-null slot wins, or `Relinquish_Default` if every slot is null).
...
4. Confirm the write path end to end on each output: **WriteProperty** a value at
   a priority, re-read `Present_Value` and `Priority_Array[priority]`, then
   **WriteProperty** `NULL` at that priority and confirm `Present_Value` falls
   back to `Relinquish_Default`.
5. Confirm a **WriteProperty** to a read-only *input* (e.g. Analog Input 1) is
   still rejected — inputs are sensors, not because nobody tried to write them.
...
```bash
python tools/gen-objects-properties.py BACnetProfileExample-B-SA-CPP            # rewrite
python tools/gen-objects-properties.py BACnetProfileExample-B-SA-CPP --check    # fail if stale
```
...
`writable` is what `SetPropertyWritable` turns on; `accepted` is for a required
property you deliberately leave to the stack's default, and each one needs a
justification. Anything required, not in `app` and not in `accepted`, comes out
as a ⚠ row - that is a defect, not a feature.
